...
export default function GrandStaff({ activeNotes, analysis, useFlat }: GrandStaffProps) {
  ...
  return (
    <div id="grand-staff-container" className="relative w-full overflow-hidden bg-[#0A0A0A] border border-white/10 shadow-lg p-6">
      ...
      <div className="w-full overflow-x-auto select-none scrollbar-none">
        ...
        >
          ...
          <text
            x="78"
            y={yTrebleCenter + 15}
            className="fill-white/20 font-serif font-bold text-6xl"
          ...
          <text
            x="78"
            y={yBassCenter - lineSpacing}
            className="fill-white/20 font-serif font-bold text-5xl"
            style={{ userSelect: 'none' }}
          >
            𝄢
          </text>
      ...
      <div className="mt-2 border-t border-white/10 pt-4 px-2">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <span className="text-[10px] font-mono tracking-[0.2em] text-white/40 uppercase font-bold flex items-center">
            <span className="w-2 h-2 rounded-full mr-2 bg-accent shadow-[0_0_6px_var(--accent)]" />
            ...
            {analysis ? (
              <>
                ...
                {bassName && rootName && bassName !== rootName && (
                  <span className="px-2 py-0.5 bg-white/5 border border-white/10 font-mono text-[10px] text-white/70">
                    <span className="text-white/40 uppercase tracking-wider mr-1 text-[8px]">Baixo:</span>
                    <strong className="text-white">{bassName}</strong>
                  </span>
                )}

                {/* Tônica */}
                {rootName && (
                  <span className="px-2 py-0.5 bg-accent/10 border border-accent/20 font-mono text-[10px] text-accent flex items-center flex-wrap gap-1">
                    <span className="text-accent/60 uppercase tracking-wider mr-1 text-[8px]">Tônica:</span>
                    <strong className="text-accent mr-1">{rootName}</strong>
                ...
                {showFifthOmitted && (
                  <span className="px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 font-mono text-[9px] font-bold text-emerald-400 uppercase tracking-wider flex items-center">
                    <span className="mr-1 text-[11px]">✔️</span> 5ª Omitida
                  </span>
                )}
              </>
            ) : (
              <span className="text-white/20 font-mono text-[9px] uppercase tracking-wider">
